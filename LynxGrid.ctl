...
'// Modified: 7/12/07
'// Version:  2.15.9a (23 May 2009)(see HistoryLog file for details)
...
Public Enum lgThemeStyleEnum
    lgTSWindows3D = 0
    lgTSWindowsFlat = 1
    lgTSWindowsTheme = 2    '// MLH - New
    lgTSOfficeXP = 3
    lgTSWindowsXP = 4
    lgTSCustom = 5       '// MLH - New
    lgTSCustom3D = 6     '// MLH - New
    lgTSVista = 7        '// MLH - New
End Enum
...
Private Const DEF_CACHEINCREMENT As Long = 5000
Private Const DEF_GRIDCOLOR As Long = &H404040
Private Const DEF_GRIDLINEWIDTH As Long = 1
Private Const DEF_PROGRESSBARCOLOR As Long = &H80FF80
...
'''Private Const DEF_THEMESTYLE            As Long = lgThemeStyleEnum.lgTSWindowsTheme
...
Private mlngRowNoWidth As Long    '// MLH - New (Show row numbers)
...
Private mBackColorSel2 As Long
...
Private mHFont As Font            '// MLH - new
...
Private mblnShowRowNo As Boolean            '// MLH - New (Show row numbers)
Private mblnShowRowNoVary As Boolean    '// MLH - New (Show row numbers)
...
Private mlTopRow As Long    '// MLH - new DrawGrid
Private mlBottomRow As Long    '// MLH - new DrawGrid
...
Public Event CellHandClick(ByVal Row As Long, ByVal Col As Long, Shift As Integer)    '// MLH - New
Public Event CellClick(ByVal Row As Long, ByVal Col As Long, Shift As Integer)    '// MLH - New
...
Public Event ItemAdded(ByVal Row As Long)
...
'---------------------------------------------------------------------------------
    ...
    If vRow = C_NULL_RESULT Then vRow = Rows
    ...
    '// MLH - mMinRowHeight is never =< 0
    'If mMinRowHeight > 0 Then
    mItems(mRowCount).lHeight = mMinRowHeight    '// set default row height
    'Else
    ...
    For lCount = 0 To lColCnt
        ...
        If lTxtCnt >= lCount Then
            ...
            If mCols(lCount).nType = lgBoolean Then    '// MLH - If Boolean set CellChecked = True
                SetFlag mItems(mRowPtr(mRowCount)).Cell(lCount).nFlags, lgFLChecked, rVal(sText(lCount))
            End If
            mudtTotalsVal(lCount) = mudtTotalsVal(lCount) + rVal(sText(lCount))    '// MLH - New
        End If
    ...
    If mbAllowWordWrap Then Call SetRowSize(mRowCount)  '// MLH - adjust row height if needed
    Call DisplayChange

    Erase sText
    RaiseEvent RowCountChanged
    RaiseEvent ItemAdded(lColling)
End Function
...
Sub ApplyCellFormat(ByVal vRow As Long, _
...
'---------------------------------------------------------------------------------
    ...
    If nNewIndex = C_NULL_RESULT Then
        ...
        With mCF(nNewIndex)
            ...
            .sFontName = sFontName
            .bHand = bHand
            .nImage = nImage    '// MLH - moved someplace else
        End With
    End If
...
Public Property Get BackColorSel2() As OLE_COLOR

    BackColorSel2 = mBackColorSel2

End Property

Public Property Let BackColorSel2(ByVal vNewValue As OLE_COLOR)

    mBackColorSel2 = vNewValue
    PropertyChanged "BackColorSel2"
    Call DisplayChange
...
Attribute CellHandPointer.VB_Description = "Returns/sets a value that determines the if the hand pointer is visible"
    ...
    CellHandPointer = mCF(mItems(mRowPtr(vRow)).Cell(vCol).nFormat).bHand

End Property

Public Property Let CellHandPointer(Optional ByVal vRow As Long, Optional ByVal vCol As Long, ByVal vNewValue As Boolean)

    If FixRef(vRow, vCol) Then    '// MLH
        ApplyCellFormat vRow, vCol, lgCFHandPointer, vNewValue
        Call DrawGrid(mbRedraw)
    End If
...
Public Property Let CellImage(Optional ByVal vRow As Long, Optional ByVal vCol As Long, ByVal vNewValue As Variant)
    ...
    If FixRef(vRow, vCol) Then    '// MLH
        ...
        Call ApplyCellFormat(vRow, vCol, lgCFImage, nImage)
        Call DrawGrid(mbRedraw)
...
Public Sub ColForceFit()
    ...
    ''' Old Code - All Columns will have the same width
    '''------------------------------------------------
    '''   '// Get number if visible columns
    '''   For lngI = 0 To lngC
    '''      If ColVisible(lngI) Then
    '''         lngVWidth = lngVWidth + 1
    '''      End If
    '''   Next lngI
    '''
    '''   dblWidth = 1 / lngVWidth
    '''   dblWidth = Round(VisibleWidth * dblWidth)
    '''
    '''   For lngI = 0 To lngC
    '''      ColWidth(lngI) = CInt(dblWidth)
    '''   Next lngI
    '''------------------------------------------------
    ...
    If mblnShowRowNo Then    '// MLH - New for "Show Row Numbers"
...
'// Purpose: The Primary Rendering routine. Draws Columns & Rows
    ...
    Dim lngBColor As Long    '// MLH
    Dim lngGColor As Long    '// MLH
    Dim lngTemp As Long    '// MLH
    ...
    If bRedraw Then
        ...
        Select Case muThemeStyle    '// MLH - New
            Case lgTSOfficeXP
        ...
        With UserControl
            ...
            If mRowCount < 0 Then
                ...
                GoTo Exit_Here  '// MLH - exit sub if there is nothing to do
            End If

            mlTopRow = SBValue(efsVertical)

            lMaxRow = mlTopRow + lRowsVisible    '// MLH - call SBValue(efsVertical) once
            ...
            bToggle = mlTopRow Mod 2

            '// Begin drawing visible rows
            For lRow = mlTopRow To mRowCount    '// MLH - was lMaxRow
                If mItems(mRowPtr(lRow)).bGroupRow Or mItems(mRowPtr(lRow)).bVisible Then  '// MLH - new
                    ...
                    If (muMultiSelect > 0 Or mbFullRowSelect) And (mItems(mRowPtr(lRow)).nFlags And lgFLSelected) Then
                        ...
                        Select Case muFocusRowHighlightStyle    '// MLH - gradient
                            ...
                            Case [Gradient_H]
                                Call FillGradient(.hDC, R, lValue, TranslateColor(mBackColorSel2), False)
                                ...
                                If lCol = 0 Then    '// Code for column 0 only (checkbox and row image)
                                    If Not mItems(mRowPtr(lRow)).bGroupRow And mbCheckboxes Then    '// Row CheckMarks?
                                        ...
                                        If Not DrawTheme("Button", 3, lValue, R) Then
                                            lngTemp = (R.Top + 14 - R.Bottom) \ 2     '// MLH
                                            If lngTemp < 0 Then
                                                R.Top = R.Top - lngTemp
                                ...
                                Select Case mCols(mColPtr(lCol)).nType
                                    Case lgBoolean
                                        If Not mItems(mRowPtr(lRow)).bGroupRow Then    '// MLH- New
                                            SetItemRect lRow, lCol + mlngRowNoWidth, lY, R, lgRTCheckBox
                                    ...
                                    Case Else
                                        If Not mItems(mRowPtr(lRow)).bGroupRow Then    '// MLH- New
                                            Select Case mCols(mColPtr(lCol)).nType
                                                ...
                                                Case lgProgressBar    '// MLH - Moved to allow Text on Progress bar
                                                    If mItems(mRowPtr(lRow)).Cell(mColPtr(lCol)).nFlags > 0 Then
                                                        lValue = ((mCols(mColPtr(lCol)).lWidth - 2) / 100) * mItems(mRowPtr(lRow)).Cell(mColPtr(lCol)).nFlags
                                                        'SetRect R, (lX + 2), (lY + 2), (lX + lValue), ((lY + mItems(mRowPtr(lRow)).lHeight) / 2 - 2)
                                                        SetRect R, (lX + 2), (lY + 2), (lX + lValue), ((lY + mItems(mRowPtr(lRow)).lHeight) - 1) - 7
                                                        SetRect R1, (lX + 2), (lY + 1) + 1, (lX + lValue), ((lY + mItems(mRowPtr(lRow)).lHeight) - 2)
                                                        'DrawRect .hdc, R, TranslateColor(mProgressBarColor), True, lgProgressBar
                                                        FillGradient .hDC, R1, mBackColorSel, mBackColorSel2, True
                                                        FillGradient .hDC, R, mBackColorSel2, mBackColorSel, True
                                                        'FillGradient .hdc, R, RGB(213, 214, 255), RGB(43, 48, 255), True
                                                        'FillGradient .hdc, R1, RGB(43, 48, 255), RGB(213, 214, 255), True
                                                    End If
                                        ...
                                        With mItems(mRowPtr(lRow)).Cell(mColPtr(lCol))
                                            ...
                                            If mbAllowWordWrap And (.nFlags And lgFLWordWrap) _
                                               And mMinRowHeightUser And Not bLineFeeds Then
                                            ...
                                            If mbAllowWordWrap And (.nFlags And lgFLWordWrap) _
                                               And (IR.Right + 6 > cWidth And mMinRowHeight < mItems(mRowPtr(lRow)).lHeight Or bLineFeeds) Then
            ...
            If mbTotalsLineShow Then
                If lRow > mRowCount Then
                    ...
                    Select Case muThemeStyle
                        ...
                        Case lgTSCustom  '// MLH - New
                            DrawXPHeader .hDC, R, 1, True, True

                        Case lgTSCustom3D    '// MLH - New
                            DrawCustom3DHeader .hDC, R, 1, True

                        Case lgTSVista    '// MLH - New
                            DrawCustom3DHeader .hDC, R, 1, True, True
...
'// the Caption and optionally Sort Arrows
    ...
    Set UserControl.Font = mHFont    '// MLH - New

    If (lCol > C_NULL_RESULT Or vblnRowNumbers) And mbColumnHeaders Then
    ...
    Set UserControl.Font = mFont    '// MLH - New
...
Private Sub DrawRect(ByVal lngHDC As Long, ByRef rc As Rect, _
                     ByVal lColor As Long, Optional ByVal bFilled As Boolean = True)
...
'// allows certain elements of the Grid to be drawn using the current Windows Theme
    ...
    If mbWinXP Then
        ...
        'If (muThemeStyle = lgTSWindowsXP) Or (muThemeStyle = lgTSOfficeXP) Then '// MLH - Draw Themed checkbox if you can
...
'// the Edit to be cancelled before anything visible occurs by setting the Cancel flag.
        ...
        If Not bCancel Then
            ...
            Else    '// External Control
                ...
                With mCols(mColPtr(mEditCol)).EditCtrl
                    ...
                    '// MLH - Caused problems with external controls
                    'sc_Subclass .hWnd
    ...
    End If
    CellProgressValue(mEditRow, mEditCol) = 0
End Sub
...
Private Sub FillGradient(ByVal lHDC As Long, _
                         ...
                         Optional ByVal bVertical As Boolean, _
                         Optional ByVal cJucko13 As Boolean = False)

    Dim pVert(0 To 2) As TRIVERTEX
    Dim pVert2(0 To 2) As TRIVERTEX
    Dim pGradRect As GRADIENT_RECT

    If cJucko13 = False Then
        With pVert(0)
            .x = rRect.Left
            .Y = rRect.Top
    ...
    Else
        ...
        '// MLH - was GradientFill
        GradientFillRect lHDC, pVert(0), 2, pGradRect, 1, IIf(Not bVertical, GRADIENT_FILL_RECT_H, GRADIENT_FILL_RECT_V)
        GradientFillRect lHDC, pVert2(0), 2, pGradRect, 1, IIf(Not bVertical, GRADIENT_FILL_RECT_H, GRADIENT_FILL_RECT_V)
        ...
        For lCol = 0 To UBound(mCols)    '// MLH - FreezeAtCol
            ...
            If lCol <= mlngFreezeAtCol Or lCol >= SBValue(efsHorizontal) Then
                ...
                With mCols(mColPtr(lCol))
                    If .bVisible Then
                        If (x > lX) And (x <= lX + .lWidth + 4 + mlngRowNoWidth) Then    '// MLH - added + 4
...
Private Function GetNewTopRow(ByVal vLastRow As Long) As Long
    ...
    '// MLH - New, used for PgUp/PgDown.  Needed because of word wrapping
...
'// Purpose: Return Row from mouse position
    ...
    If mR.HeaderHeight > 0 Then
        lColumnHeadingHeight = mR.HeaderHeight    'GetColumnHeadingHeight()
...
Public Sub RemoveRow(Optional ByVal vRow As Long = C_NULL_RESULT)
    ...
    If mRowCount >= 0 Then
        RaiseEvent BeforeDelete(vRow, False)
        ...
        If mItems(mRowPtr(lNewRow)).bVisible And blnColOk Then
            ...
            If lNewRow <= mRowCount And lNewRow >= 0 Then
                ...
                If bRedraw Or SetRowCol(lNewRow, lNewCol, True) Then
                    mRow = lNewRow
                    If mRow < mlTopRow Then    '// MLH - was < SBValue(efsVertical) Then
                        SBValue(efsVertical) = mRow
                    ElseIf lNewRow > mlBottomRow Then    '// MLH - was > (SBValue(efsVertical) + RowsVisible()) Then
                        SBValue(efsVertical) = mRow
                    End If
...
'// MLH - renamed
    ...
    With UserControl
        'RowsVisible = (.ScaleHeight - GetColumnHeadingHeight() - (lBorderWidth * 2)) / mMinRowHeight '// MLH
...
'// MLH - was public
    ...
    If Index >= lStart Then
        lY = mR.HeaderHeight    '// MLH - was GetColumnHeadingHeight()
...
Public Function MaxOutScrollBarPos(Optional REW As Integer, Optional Ass As Boolean = False)
    If Ass = False Then
        SBValue(efsVertical) = SBMax(efsVertical)
    Else
        SBValue(efsVertical) = REW
    End If
End Function
...
Private Property Let SBValue(ByVal eBar As EFSScrollBarConstants, ByVal iValue As Long)
    ...
    Dim tSI As SCROLLINFO
    If iValue = -1 Then Exit Property
...
Property Get SBVisible(ByVal eBar As EFSScrollBarConstants) As Boolean
...
Property Let SBVisible(ByVal eBar As EFSScrollBarConstants, ByVal bState As Boolean)
...
Private Sub SetItemRect(ByVal vRow As Long, _
    ...
    Call SetRect(R, lLeft + mlngRowNoWidth, lTop, lLeft + lWidth, lTop + lHeight)
...
'// Purpose: Sets the visibilty of scroll bars and sets max scroll values
    ...
    If Not (mSBhWnd = 0) Then
        ...
        lHeight = mR.HeaderHeight + UserControl.TextHeight(C_CHECKTEXT)    '// MLH
        ...
        If lHeight > UserControl.ScaleHeight Then
            ...
            lHeight = mR.HeaderHeight    '// MLH - was GetColumnHeadingHeight()
...
'// quantity and return the result
    ...
    ElseIf vlngValue < 0 Then    'NOT VLNGVALUE...
...
'// Purpose: Sort Grid based on current Sort Columns.
    ...
    If UpdateCell() Then
        ...
        'call DrawGrid( mbRedraw) '// MLH

        RaiseEvent SortComplete
    End If

    DoEvents    '// MLH - added to give the system time to update
        ...
        If Not bCancel Then
            ...
            Else
                ...
                With mCols(mColPtr(mEditCol)).EditCtrl
                    ...
                    '// MLH - Caused problems - Subclassing removed in EditCell
                    'sc_UnSubclass .hWnd
            ...
            If bRequestUpdate Then
                ...
                SetFlag mItems(mRowPtr(mEditRow)).Cell(mColPtr(mEditCol)).nFlags, 0, True
...
Private Sub UserControl_InitProperties()
    ...
    mBackColor = &H80000006
    mBackColorBkg = &H0&
    mBackColorEdit = &H8000000A
    ...
    muFocusRectStyle = lgFocusRectStyleEnum.lgFRLight
    muGridLines = lgGridLinesEnum.lgGrid_Both
    mblnColumnHeaderSmall = True
    mGridLineWidth = DEF_GRIDLINEWIDTH
    muThemeStyle = lgTSCustom
    mbColumnHeaders = True    '// MLH
    mbCenterRowImage = True    '// MLH
    muSBOrienation = ScrollBarOrienationEnum.Scroll_Vertical    '// MLH
    mMinVerticalOffset = DEF_MinVerticalOffset    '// MLH
    mColumnHeaderLines = 2    '// MLH
    msCaption = "Actions of your recorded data"    '// MLH
    ...
    muBorderStyle = lgBorderStyleEnum.lgNone
    mbCheckboxes = False
    muEditTrigger = lgEditTriggerEnum.lgEnterKey
    mbFullRowSelect = True
    muFocusRowHighlightStyle = lgFocusRowHighlightStyle.Gradient_H    '// MLH - new
    mbHideSelection = False
    mbAllowColumnHover = True
    muMultiSelect = lgMultiSelectEnum.lgMultiStandard    '// MLH - Added New Options
    mbRedraw = True    '// MLH
    ...
    mMinRowHeight = 10    'mMinRowHeightUser

    mBackColorEvenRows = &H1F1F1F                      '// MLH
    mbBackColorEvenRowsE = True                     '// MLH
    mlngCustomColorFrom = TranslateColor(&HC0C0C0)     '// MLH
    mlngCustomColorTo = TranslateColor(&H404040)       '// MLH
    mlngCustomColorFrom = mlngCustomColorFrom       '// MLH
    mlngCustomColorTo = mlngCustomColorTo           '// MLH
    muThemeColor = CustomTheme                      '// MLH
...
Private Sub UserControl_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        ...
        Case vbKeyC, vbKeyV    '// Allow Ctrl+C for copy; Ctrl+V for paste '// MLH
        ...
        Case vbKeyUp
            ...
            If UpdateCell() Then
                ...
                If Not lNewRow = C_NULL_RESULT Then
                    ...
                    If lNewRow < mlTopRow Or lNewRow > mlBottomRow Then    '// MLH - New
            ...
            If Not mbEditPending Then
                If UpdateCell() Then
                    If Not mRow = C_NULL_RESULT Then
                        ...
                        'lNewRow = (SBValue(efsVertical) - RowsVisible()) + 2
                        lNewRow = GetNewTopRow(mlTopRow)    '// MLH
        ...
        Case vbKeyPageDown
            ...
            If Not mbEditPending Then
                If UpdateCell() Then
                    If mRow < mRowCount Then
                        'lNewRow = (mRow + RowsVisible()) - 1 '// MLH - prevent selection change
                        'lNewRow = (SBValue(efsVertical) + RowsVisible()) - 2'// MLH
                        lNewRow = mlBottomRow    '// MLH
...
Private Sub UserControl_MouseMove(Button As Integer, Shift As Integer, x As Single, Y As Single)
    ...
    If Not (mRowCount = C_NULL_RESULT) Then
        ...
        If Not (mMouseRow = C_NULL_RESULT) Then    '// mouse on header row
            If mCF(mItems(mRowPtr(mMouseRow)).Cell(mMouseCol).nFormat).bHand Then
...
Private Sub UserControl_MouseUp(Button As Integer, Shift As Integer, x As Single, Y As Single)
    ...
    If (Button = vbLeftButton) Then
        ...
        ElseIf Not (mResizeCol = C_NULL_RESULT And mResizeRow = C_NULL_RESULT) Then    '// MLH
            ...
            Call SetScrollBars
            Call DrawGrid(mbRedraw)
            '''         UserControl.MousePointer = vbDefault '// MLH Removed - prevented auto-resize on doubleclick
    ...
    If Not (mMouseRow = C_NULL_RESULT Or mMouseCol = C_NULL_RESULT) Then    '// MLH - New
        ...
        If HandCursorVisible Then RaiseEvent CellHandClick(mMouseRow, mColPtr(mCol), mnShift)    '// MLH - New
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        mBackColorSel2 = .ReadProperty("BackColorSel2", vbHighlight)
        ...
        mblnShowRowNo = .ReadProperty("ShowRowNumbers", False)    '// MLH - new
        mblnShowRowNoVary = .ReadProperty("ShowRowNumbersVary", True)    '// MLH - new
        ...
        '      Set Font = .ReadProperty("Font", Ambient.Font) '// MLH - Causes call to CreateRenderData
        Set mFont = .ReadProperty("Font", Ambient.Font)
        Set UserControl.Font = .ReadProperty("Font", Ambient.Font)
        Set mHFont = .ReadProperty("FontHeader", mFont)    '// MLH - New
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Call .WriteProperty("FontHeader", mHFont, Ambient.Font)    '// MLH - New
        ...
        Call .WriteProperty("BackColorSel2", mBackColorSel2, vbHighlight)
        ...
        Call .WriteProperty("ShowRowNumbers", mblnShowRowNo, False)    '// MLH
        Call .WriteProperty("ShowRowNumbersVary", mblnShowRowNoVary, False)    '// MLH
    ...
    Select Case uMsg
        ...
        Case WM_MOUSELEAVE
            If mbInCtrl Then
                mbInCtrl = False
                picTooltip.Visible = False    '// MLH
                HandCursorVisible = False    '// MLH - New
                UserControl.MousePointer = vbDefault    '// MLH - New
                Call DrawHeaderRow
                UserControl.Refresh
